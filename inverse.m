...
##   [xpos,ypoz] = inverse(xGuess, yGuess, &guessLengthA, &guessLengthB)
...
function   [LengthA, LengthB] = inverse(xpos, ypos,type)
  ...
  workspaceWidth = 1246; ## mm
  
  ## My 11.5 feet wide maslow
  #distanceBetweenLRShafts = 3501.2; ## mm 
  #workspaceTopToShaftsHeight = 618; ## mm // My 11.5 feet wide maslow
  #maxTopBeamTipFlexAndTwist = 0; ## mm
  #sledRotationRadius = 138.14; ## mm
  #sledYOffsetAboveBit = 0;# -0.25; ## mm
  #sledXOffsetRightOfBit = 0;# -0.75; ## mm
  #leftChainLengthCorrectionRatio = 1.00; ##.0013; ## ratio
  #rightChainLengthCorrectionRatio = 1.00; ## ratio
  #sledMass = 11.6;
  ...
  maxTopBeamTipFlexAndTwist = 0; #2.9; ## mm // assume a rigid beam for now...
  sledRotationRadius = 138.14; ## mm
  ...
  chainElongationFactor = 8.1E-6; ## fraction added / N 2.6mm / 0.640kN /1000 mm/m
  ...
  cheatChainElongationFactor = 8.1E-6; # kg/m is the integral value used in the inverse calculation
  ...
  ## Compensations parameters to try to correct position within existing Maslow settings
  cheatLRDistance2 = 0.0; #2.5; ##(mm) reduction to straighten the top bow
  cheatLRMotorsYOffsetAboveWorkSurface2 = 0; #2.5; ##(mm) increase to raise beam too high
  cheatRotationRadius2 = -0.0; #2.5; ##(mm) reduction to have a short radius
  cheatLeftChainLength2 = -0.5; #2.5; ##(mm) reduction to have a short chain
  cheatRightChainLength2 = -0.5; #2.5; ##(mm) reduction to have a short chain
  cheatChainDensity2 = 14.06E-2; # 1E-6; # kg/m is the integral value used in the inverse calculation
  cheatChainWeightDensity2 = cheatChainDensity2 * accG;
  cheatChainElongationFactor2 = 8.1E-6; # kg/m is the integral value used in the inverse calculation
  cheatLeftChainElongationFactor2 = 0; # kg/m is the value added to the right chain cheatChainElongationFactor2
  cheatRightChainElongationFactor2 = 0;#3E-6; # kg/m is the value added to the right chain cheatChainElongationFactor2
  cheatleftChainLengthCorrectionRatio2 = +0.0000; ## 0.0013 adds to leftChainLengthCorrectionRatio
  cheatrightChainLengthCorrectionRatio2 = 0.0000; ## 0.0013 adds to rightChainLengthCorrectionRatio
  cheatmaxTopBeamTipFlexAndTwist2 = 0; # mm added to the beam flex maxTopBeamTipFlexAndTwist
  ...
  case "allcatenary_with_error2"
    # use distanceBetweenLRShafts-cheatLRDistance
    
    ## initial geometry and chains lenght estimation without sag -> call first to gete chain angles estimations
    [leftChainExtent_raw, rightChainExtent_raw, leftChainStraightLength_raw,rightChainStraightLength_raw, leftStraightChainAngle, rightStraightChainAngle, leftChainAttachX, leftChainAttachY, rightChainAttachX, rightChainAttachY] = computeNoWeightChains (SprocketRadius , sledX, sledY, distanceBetweenLRShafts-cheatLRDistance2,maxTopBeamTipFlexAndTwist+cheatmaxTopBeamTipFlexAndTwist2, originY+cheatLRMotorsYOffsetAboveWorkSurface2);
    ...
    ## change coordinate system for sag calculation 
    targetSledX = sledX;
    targetSledY = originY+cheatLRMotorsYOffsetAboveWorkSurface2-sledY; ## mm
    leftChainAttachY = originY+cheatLRMotorsYOffsetAboveWorkSurface2 - leftChainAttachY;
    rightChainAttachY = originY+cheatLRMotorsYOffsetAboveWorkSurface2 - rightChainAttachY;
    
    ## establish the horizontal chain attach points distance. This is a target for the sag calculations.
    ChainAttachpointsSeparation = rightChainAttachX - leftChainAttachX;
    
    ## lauch the chain sag calculation for that sled coordinate
    [longueur_ideale_suspendue_gauche,longueur_ideale_suspendue_droite, leftChainAttachY, rightChainAttachY, largeur_entre_points, angle_gauche, angle_droit, angleSledGauche, angleSledDroit, leftChainT, rightChainT, catenary_a, erreur_sledX, iteration] = computeRealWeightChains (targetSledX, targetSledY, leftChainAttachX, leftChainAttachY, rightChainAttachX, rightChainAttachY, leftChainStraightLength, rightChainStraightLength, cheatChainWeightDensity2, sledWeight, maxSagError, maxWidthError, maxIteration);
    #longueur_ideale_suspendue_gauche = 1;
    #longueur_ideale_suspendue_droite = 1;
    #largeur_entre_points = ChainAttachpointsSeparation;
    #angle_gauche = pi/3;
    ...
    ## adapt chain length to fit calibration parameters
    ## include chain stretch and pitch error
    leftChainElongationFactor = 1+ (leftChainT * (cheatChainElongationFactor2+cheatLeftChainElongationFactor2));
    leftChainSuspendedSegmentLength = longueur_ideale_suspendue_gauche / (leftChainElongationFactor * (leftChainLengthCorrectionRatio+cheatleftChainLengthCorrectionRatio2));
    
    rightChainElongationFactor = 1+ (rightChainT * (cheatChainElongationFactor2+ cheatRightChainElongationFactor2));
    rightChainSuspendedSegmentLength = longueur_ideale_suspendue_droite / (rightChainElongationFactor * (rightChainLengthCorrectionRatio+cheatrightChainLengthCorrectionRatio2));
    ...
    ## get raw length before triangular link radius 
    LengthA = leftChainExtent-cheatRotationRadius2+cheatLeftChainLength2;
    LengthB = rightChainExtent-cheatRotationRadius2+cheatRightChainLength2;
  endswitch
